...
contract MerchToken is ERC721, Ownable {
    ...
    function changePrice(uint256 _tokenId, uint256 _newPrice) public {
        require(_newPrice != 0); // ensure token price is non-zero
        require(_tokenId < index); // ensure tokenId exists
        require(ownerOf(_tokenId) == msg.sender); // ensure only the token owner can call function
